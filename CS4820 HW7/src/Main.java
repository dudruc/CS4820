...
class Main {
	...
	public Main() {
		...
		int[][] graph = new int[numBlockH * numBlockW + 2][numBlockH * numBlockW + 2];
		...
		for (int BW = 1; BW < numBlockW; BW++) {
			for (int BH = 0; BH < numBlockH; BH++) {
				...
				graph[BH * numBlockW + BW][BH * numBlockW + BW - 1] = (int) Math.floor(pBlkBtwColsSum / blockHeight);
				graph[BH * numBlockW + BW - 1][BH * numBlockW + BW] = (int) Math.floor(pBlkBtwColsSum / blockHeight);
			}
		}
